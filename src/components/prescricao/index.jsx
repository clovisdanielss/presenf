...
class Prescricao extends Component {
  ...
  onSelectValue(value, index, param) {
    ...
    for (var i = 0; i < this.state.diagnosticos.length; i++) {
      if (
        param === "Intervenção" ||
        this.state.diagnosticos[i].index === parseInt(index)
      ) {
  ...
  loadEnfermeiro() {
    ...
    enfReq.open(
      "GET",
      process.env.REACT_APP_URL + "enfermeiro/" + this.props.prescricao.coren
    );
  ...
  loadLoggedEnfermeiro() {
    ...
    enfReq.open(
      "GET",
      process.env.REACT_APP_URL + "enfermeiro/" + this.context.enfermeiro.coren
    ); // TODO enfermeiro temporario usado no login
    enfReq.setRequestHeader("Authorization", this.context.enfermeiro.token);
  ...
  loadPrescricaoRecente() {
    ...
    prescricoesReq.setRequestHeader(
      "Authorization",
      this.context.enfermeiro.token
    );
    ...
    if (this.state.enfermeiro && this.props.readOnly) {
      return (
        <PrescricaoReadOnly
          ...
          prescricao={this.props.prescricao}
        />
      );
